...
export class RulesService {
   ...
   * 
   * @param scannerType - The scanner type (HTMLCS or Axe)
  ...
  async getRulesForScanner(
    ...
    this.logger.log(`Loading rules for scanner: ${scannerType}, language: ${language}`);

    try {
      // Map scanner type to Kayle runner name
      const runnerName = this.mapScannerTypeToRunner(scannerType);
      
      // Load rules from Kayle
      const kayleRules: KayleRule[] = await importRules(language, runnerName);
      
      this.logger.log(`Loaded ${kayleRules.length} rules for ${scannerType}`);

      // Get the appropriate rule service for URL generation
      const ruleService = this.ruleServiceFactory.getRuleService(scannerType);
      ...
      this.logger.error(`Failed to load rules for scanner ${scannerType}:`, error);
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      throw new Error(`Failed to load rules for scanner ${scannerType}: ${errorMessage}`);
    }
  }
